...
#Region " ITeamServerAdmin "
    ...
    Private _TeamServerAdminCallback As ITeamServerAdminCallback

    Public ReadOnly Property TeamServerAdminCallback() As ITeamServerAdminCallback
        Get
            If _TeamServerAdminCallback Is Nothing Then
                _TeamServerAdminCallback = OperationContext.GetCallbackChannel(Of ITeamServerAdminCallback)()
            End If
            Return _TeamServerAdminCallback
        End Get
    End Property

    Public Function ServceUrl() As System.Uri Implements ITeamServerAdmin.ServceUrl
        Return OperationContext.EndpointDispatcher.EndpointAddress.Uri
    End Function

    Public Sub AddServer(ByVal TeamServerName As String, ByVal TeamServerUri As String) Implements ITeamServerAdmin.AddServer
        Try
    ...
    Public Sub RemoveServer(ByVal TeamServerName As String) Implements ITeamServerAdmin.RemoveServer
        Try
    ...
    Public Function GetServers() As String() Implements ITeamServerAdmin.GetServers
...
#Region " ISubscriptionAdmin"
    ...
    Private _SubscriptionAdminCallback As ISubscriptionAdminCallback

    Public ReadOnly Property SubscriptionAdminCallback() As ISubscriptionAdminCallback
        Get
            If _SubscriptionAdminCallback Is Nothing Then
                _SubscriptionAdminCallback = OperationContext.GetCallbackChannel(Of ISubscriptionAdminCallback)()
            End If
            Return _SubscriptionAdminCallback
        End Get
    End Property

    Public Sub AddSubscriptions(ByVal ServiceUrl As String, ByVal EventType As EventTypes) Implements ISubscriptionAdmin.AddSubscriptions
        Try
    ...
    Public Sub RemoveSubscriptions(ByVal ServiceUrl As String) Implements ISubscriptionAdmin.RemoveSubscriptions
        Try
            For Each TeamServerName As String In Me.GetServers()
                Dim tfs As TeamFoundationServer = Me.GetTeamServer(TeamServerName)
                Dim EventService As IEventService = CType(tfs.GetService(GetType(IEventService)), IEventService)
    ...
    Public Function GetSubscriptions() As System.Collections.ObjectModel.Collection(Of Subscription) Implements ISubscriptionAdmin.GetSubscriptions
        Dim Subscriptions As New Collection(Of Subscription)
        For Each TeamServerName As String In Me.GetServers()
            Dim ServerSubs() As Server.Subscription = GetServerSubs(TeamServerName)
            For Each serverSub As Server.Subscription In ServerSubs
...
#Region " INotificationAdmin "
    ...
    Public Function GetLocal() As String Implements INotificationAdmin.GetLocal
        ...
        ' Dim x As Reflection.Assembly = System.Reflection.Assembly.GetExecutingAssembly
